# Branch audit

Repository: `MachineLearning-Nerd/icml26-rdpg-spectral-embedding`

The legacy repository contained one branch. The branch was renamed for a clean public history, and the final branch is intended to be the only active ref for this reproduction.

| Legacy ref | Source tip before normalization | Clean ref | Role |
|---|---|---|---|
| `master` | `eae61d72c3e41df3744ca7a4d1416aaa7cb93f7c` | `main` | Default documented reproduction audit |

## Final verification contract

- `main` is the default branch.
- No `master` or `orx/*` branch remains.
- `README.md`, `STATUS.md`, `GATE_READY.md`, and this branch audit are present on `main`.
- Reachable commit author and committer identities are normalized to `MachineLearning-Nerd <37579156+MachineLearning-Nerd@users.noreply.github.com>`.
- The repository name and active links use `icml26-rdpg-spectral-embedding`.
